...
PROGMEM uint8_t keepAlive[] = {0x03, 0x5B, 0x01, 0x42}; // Only Hero 9 and 10 ? Hero 8 response is Error
...
PROGMEM uint8_t shutterOff[] = {0x03, 0x01, 0x01, 0x00};
PROGMEM uint8_t shutterOn[] = {0x03, 0x01, 0x01, 0x01};
PROGMEM uint8_t camSleep[] = {0x01, 0x05};
PROGMEM uint8_t wiFiAPoff[] = {0x03, 0x17, 0x01, 0x00};
PROGMEM uint8_t wiFiAPon[] = {0x03, 0x17, 0x01, 0x01};
PROGMEM uint8_t hilight[] = {0x01, 0x18};
PROGMEM uint8_t videoGroup[] = {0x04, 0x3E, 0x02, 0x03, 0xE8};
PROGMEM uint8_t photoGroup[] = {0x04, 0x3E, 0x02, 0x03, 0xE9};
PROGMEM uint8_t timelapseGroup[] = {0x04, 0x3E, 0x02, 0x03, 0xEA};
PROGMEM uint8_t standard[] = {0x06, 0x40, 0x04, 0x00, 0x00, 0x00, 0x00};
PROGMEM uint8_t activity[] = {0x06, 0x40, 0x04, 0x00, 0x00, 0x00, 0x01};
PROGMEM uint8_t cinematic[] = {0x06, 0x40, 0x04, 0x00, 0x00, 0x00, 0x02};
PROGMEM uint8_t photo[] = {0x06, 0x40, 0x04, 0x00, 0x01, 0x00, 0x00};
PROGMEM uint8_t liveBurst[] = {0x06, 0x40, 0x04, 0x00, 0x01, 0x00, 0x01};
PROGMEM uint8_t burstPhoto[] = {0x06, 0x40, 0x04, 0x00, 0x01, 0x00, 0x02};
PROGMEM uint8_t nightPhoto[] = {0x06, 0x40, 0x04, 0x00, 0x01, 0x00, 0x03};
PROGMEM uint8_t timeWarp[] = {0x06, 0x40, 0x04, 0x00, 0x02, 0x00, 0x00};
PROGMEM uint8_t timeLapse[] = {0x06, 0x40, 0x04, 0x00, 0x02, 0x00, 0x01};
PROGMEM uint8_t nightLapse[] = {0x06, 0x40, 0x04, 0x00, 0x02, 0x00, 0x02};

/*
 * Strings
 */
const PROGMEM char hero_10_black[] = "Hero 10 Black";
const PROGMEM char hero_9_black[] = "HERO 9 Black";
const PROGMEM char hero_max[] = "MAX";
const PROGMEM char hero_8_black[] = "HERO 8 Black";
const PROGMEM char hero_7_white[] = "HERO 7 White";
const PROGMEM char hero_7_silver[] = "HERO 7 Silver";
const PROGMEM char hero_2018[] = "HERO (2018)";
const PROGMEM char hero_7_black[] = "HERO 7 Black";
const PROGMEM char hero_6_black[] = "HERO 6 Black";
const PROGMEM char hero_5_session[] = "HERO 5 Session";
const PROGMEM char hero_fusion[] = "Fusion";
const PROGMEM char hero_p[] = "HERO +";
const PROGMEM char hero_5_black[] = "HERO 5 Black";
const PROGMEM char hero_p_lcd[] = "HERO + LCD";
const PROGMEM char hero_4_session[] = "HERO 4 Session";
const PROGMEM char hero_4_silver[] = "HERO 4 Silver";
const PROGMEM char hero_4_black[] = "HERO 4 Black";
const PROGMEM char hero_2014[] = "HERO (2014)";
const PROGMEM char hero_3p_black[] = "HERO 3+ Black";
const PROGMEM char hero_3p_silver[] = "HERO 3+ Silver";
const PROGMEM char hero_3_black[] = "HERO 3 Black";
const PROGMEM char hero_3_silver[] = "HERO 3 Silver";
const PROGMEM char hero_3_white[] = "HERO 3 White";
const PROGMEM char hero_unknown[] = "UNKNOWN";
const PROGMEM char text_plain[] = "text/plain";
...
void sendCommand(uint8_t *command)
{
  ...
  for (uint8_t i = 0; i < maxCams; i++)
  {
    if (isCamAvailable(i))
    {
      goProCams[i].pClient->getService(controlServiceUUID)->getCharacteristic(commandUUID)->writeValue(command, sizeof(command), false);
...
void heartBeat(uint8_t camIndex)
{
  ...
  {
    const char *value = goProCams[camIndex].pClient->getService(battInfoUUID)->getCharacteristic(battLevelUUID)->readValue().c_str();
    goProCams[camIndex].battLevel = String(value[0], DEC);

    // send keep alive only for Hero 9 and newer
    if (goProCams[camIndex].modelID >= 22)
    {
      goProCams[camIndex].pClient->getService(controlServiceUUID)->getCharacteristic(settingsUUID)->writeValue(keepAlive, sizeof(keepAlive), false); // writeValue(uint8_t *data, size_t length, bool response)
    }
...
void getCamModel(uint8_t camIndex)
{
  ...
  else if (goProCams[camIndex].serialNo[0] == 'H')
  {
    if (goProCams[camIndex].serialNo[1] == '3')
    {
      ...
      {
        // HERO 3+ Black    H3B~
        goProCams[camIndex].modelName = hero_3p_black;
        goProCams[camIndex].modelID = 5;
      }
      else if (goProCams[camIndex].serialNo[2] == 'S')
      {
        // HERO 3+ Silver H3S~
        goProCams[camIndex].modelName = hero_3p_silver;
...
static void scanCompleteCB(BLEScanResults scanResults)
{
  ...
  for (int ai = 0; ai < scanResults.getCount(); ai++)
  {
    if (scanResults.getDevice(ai).haveServiceUUID() && scanResults.getDevice(ai).isAdvertisingService(controlServiceUUID))
    {
      ...
      for (uint8_t i = 0; i < maxCams; i++)
      {
        if (goProCams[i].pDevice != nullptr && goProCams[i].isConnected)
        {
...
bool connectToCam(uint8_t camIndex)
{
  ...
  goProCams[camIndex].pClient->getService(controlServiceUUID)->getCharacteristic(commandRespUUID)->registerForNotify(notifyCallback);
  goProCams[camIndex].pClient->getService(controlServiceUUID)->getCharacteristic(settingsRespUUID)->registerForNotify(notifyCallback);

  const char *value1 = goProCams[camIndex].pClient->getService(defInfoUUID)->getCharacteristic(serialNoUUID)->readValue().c_str();
  goProCams[camIndex].serialNo = (char *)value1;
  ...
  const char *value2 = goProCams[camIndex].pClient->getService(battInfoUUID)->getCharacteristic(battLevelUUID)->readValue().c_str();
  goProCams[camIndex].battLevel = String(value2[0], DEC);

  goProCams[camIndex].lastKeepAlive = millis();
  M5.dis.drawpix(0, 0xfff000); // YELLOW indicates successful Connection 
#ifdef DEBUG
...
void setup()
{
  ...
  pBLEScan->start(scanSeconds, &scanCompleteCB);
  Serial.printf("FreeHeap after Setup: %d byte\n", ESP.getFreeHeap());
}
...
#ifdef DEBUG
            Serial.printf("FreeHeap after Cam connected: %d byte\n", ESP.getFreeHeap());
#endif
          }
          else
          {
...
/////////////////////////////////////////////////////////////////////////
        ...
        if (millis() - goProCams[i].lastKeepAlive > 30000 && goProCams[i].isConnected)
        {
          heartBeat(i);
#ifdef DEBUG
          Serial.printf("[heartBeat] -> was sent to Cam %d\n", i);
          Serial.printf("FreeHeap: %d byte\n", ESP.getFreeHeap());
#endif
        }
      }
    }
